...
app.post("/jokes", (req, res) => {
  ...
  const newIndex = jokes[jokes.length - 1].id + 1
  if (jokes.length === 0) newIndex = 1
  const newJoke = {id:newIndex, jokeText:content, jokeType:type}
  jokes.push(newJoke)
  res.json(newJoke)
})
...
app.put("/joke/:id", (req, res) => {
  // console.log(req.params.id);
  const newJoke = req.body.content
  ...
  if(joke) {
    joke.jokeText = newJoke
    joke.jokeType = newJokeType
    jokes[index] = joke
    res.json(joke);
  } else {
    res.json("Error fetching data")
...
app.delete("/jokes/all", (req, res) => {
  ...
  if(master == masterKey) {
    ...
    res.json("Master keys do not match")
